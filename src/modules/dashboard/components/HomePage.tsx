...
export const HomePage: React.FC<{ currentRoute?: string }> = () => {
  ...
  return (
    <div className={`h-screen w-screen overflow-hidden flex bg-white dark:bg-[#0e0f13] m-0 p-0 font-['Outfit',sans-serif] ${isDark ? 'dark' : ''}`}>
      ...
      <aside className={`h-full shrink-0 relative z-20 flex flex-col bg-white dark:bg-black transition-[width] duration-200 ease-out overflow-hidden ${isExpanded ? 'w-[224px]' : 'w-[115px]'}`}>
        ...
        <div className={`flex flex-col flex-1 ${isExpanded ? 'py-[28px] px-5 items-stretch' : 'py-6 items-center'}`}>
          ...
          >
            ...
            <img src={isDark ? logoFull : logoFullLight} alt="RightStepAhead" className={`h-7 w-auto ${isExpanded ? 'block' : 'hidden'}`} />
          </button>
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col relative z-30 h-full border-l border-slate-300 dark:border-[#272735]">
        ...
        {/* Title Bar */}
        <header className="h-[67px] shrink-0 w-full relative z-10 flex items-center justify-end bg-slate-50 dark:bg-[#0e0f13]">
        ...
        {/* Main Content Pane */}
        <main className="flex-1 bg-slate-50 dark:bg-[#0e0f13] relative overflow-hidden">
          
          {selectedNav === 'home' && (
            <div className="w-full h-full bg-slate-50 dark:bg-[#0e0f13]"></div>
          )}

          {selectedNav === 'pathway' && !isPathwayActive && (
            <section className="absolute inset-0 overflow-hidden bg-slate-50 dark:bg-[#0e0f13] flex flex-col justify-center responsive-scale">
              <img className="absolute inset-0 w-full h-full object-cover object-right opacity-90 dark:opacity-90 opacity-20" src={pathwayBg} alt="" />
              <div className="absolute inset-0 bg-gradient-to-r from-slate-50 dark:from-[#0e0f13] via-slate-50/90 dark:via-[#0e0f13]/90 via-[45%] to-slate-50/5 dark:to-[#0e0f13]/5" />
              ...
              <div className="relative px-6 md:px-12 lg:px-[96px] w-full max-w-[900px] z-10">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-['Outfit',sans-serif] font-medium text-[16px] lg:text-[20px] text-slate-600 dark:text-[#9090b0]">Hi Mohit</span>
                </div>
                <p className="mt-3 lg:mt-4 font-['Outfit',sans-serif] font-light text-[18px] md:text-[20px] lg:text-[22px] leading-[1.35] text-slate-900 dark:text-white">
